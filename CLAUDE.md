...
- [src/components/date-card.tsx](src/components/date-card.tsx): Per-date card.
  ...
  - Each group's `TaskSelect` shows an info tooltip: the group's `description` plus, for the Jira-backed three, the literal JQL that produced it. The tooltip reads the same `jqlByGroup` strings the queries ran, so displayed and executed JQL cannot drift; favorites have no entry and so show no JQL.
